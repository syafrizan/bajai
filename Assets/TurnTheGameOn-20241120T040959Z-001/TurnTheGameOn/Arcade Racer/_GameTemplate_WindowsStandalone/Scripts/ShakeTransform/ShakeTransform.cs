namespace TurnTheGameOn.ArcadeRacer
{
    ...
    public class ShakeTransform : MonoBehaviour
    {
        public class ShakeEvent
        {
            ...
            public ShakeSettings.Target target
            {
                get
                {
                    return data.target;
            ...
            public ShakeEvent(ShakeSettings _data)
            {
                this.data = _data;
        ...
        public void AddShakeEvent(ShakeSettings data)
        {
            shakeEvents.Add(new ShakeEvent(data));
        }

        public void AddShakeEvent(float _amplitude, float _frequency, float _duration, AnimationCurve _blendOverLifetime, ShakeSettings.Target _target)
        {
            ShakeSettings data = ScriptableObject.CreateInstance<ShakeSettings>();
        ...
        private void LateUpdate()
        {
            ...
            for (int i = shakeEvents.Count - 1; i != -1; i--)
            {
                ...
                if (shakeEvent.target == ShakeSettings.Target.Position)
                {
