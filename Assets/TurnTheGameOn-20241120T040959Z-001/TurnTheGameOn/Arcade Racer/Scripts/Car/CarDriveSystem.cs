namespace TurnTheGameOn.ArcadeRacer
{
    ...
    public class CarDriveSystem : MonoBehaviour
    {
        private CarPlayerInput playerInput;
        public CarUIManager vehicleUI;
        public CarSettings vehicleSettings;
        public CarSettings _vehicleSettings { get; private set; }
        public CarWheelSettings wheelSettings;
        public Rigidbody _rigidbody { get; private set; }
        public Transform _transform { get; private set; }
        public CarWheelInfo[] wheels;
...
#pragma warning restore 0067
        ...
        private void Awake()
        {
            ...
            playerInput = GetComponent<CarPlayerInput>();
        ...
        public void Move(float steering, float accel, float footbrake, float handbrake)
        {
            ...
            ApplyDrive(accel, footbrake);
            if (vehicleSettings.enableDragOverride) DragOverride(accel, steering);
            CapSpeed();
        ...
        private void UpdateGear()
        {
            ...
            var revsRangeMin = Mathf.Lerp(0f, 1.0f, 1 - (1 - gearNumFactor) * (1 - gearNumFactor));  //1 - (1 - gearNumFactor)*(1 - gearNumFactor); // simple function to add a curved bias towards 1 for a value in the 0-1 range
            var revsRangeMax = Mathf.Lerp(1.0f, 1f, gearNumFactor);
            Revs = Mathf.Lerp(revsRangeMin, revsRangeMax, 0.0f);
            ...
            var revsRangeMinA = Mathf.Lerp(0f, 1.0f, 1 - (1 - gearNumFactorA) * (1 - gearNumFactorA));  //1 - (1 - gearNumFactor)*(1 - gearNumFactor); // simple function to add a curved bias towards 1 for a value in the 0-1 range
            var revsRangeMaxA = Mathf.Lerp(1.0f, 1f, gearNumFactor);
            RevsAudio = Mathf.Lerp(revsRangeMinA, revsRangeMaxA, gearFactor);
            if (!_vehicleSettings.manual)
            {
        ...
        void ApplyBrake(float _brakeAmount)
        {
            ...
            {
                isBraking = true;
                OnSetIsBraking?.Invoke();
            }
            else if (isBraking && _brakeAmount == 0.0f)
            {
                isBraking = false;
                OnSetIsNotBraking?.Invoke();
            }
            canReverse = false;
            for (int i = 0; i < 4; i++)
            {
        ...
        void CheckCanReverse()
        {
            if (templateCar)
            {
                if (playerInput)
                {
                    ...
                    if (GameTemplate_Player.Instance.playerInputSettings.inputAxes.releaseBrakeToReverse)
                    {
            ...
            else
            {
                ...
                if (playerInput)
                {
                    if (playerInput.playerInputSettings.inputAxes.releaseBrakeToReverse)
                    {
                        if (Input.GetKeyUp("down"))
                        {
                            Invoke("SetCanReverse", 0.1f);
        ...
        private void UpdateWheels(float _steerInput, float _handBrake)
        {
            ...
            if (overrideSteering) _steerInput = overrideSteeringPower;
        ...
        private void DragOverride(float _throttleInput, float _steerinput)
        {
            if (_throttleInput > 0) // apply min drag when acceleration is used
            {
                _rigidbody.drag = vehicleSettings.minDrag;
                _rigidbody.angularDrag = vehicleSettings.minAngularDrag;
            }

            else // apply max drag when not accelerating
            {
                _rigidbody.drag = vehicleSettings.maxDrag;
                _rigidbody.angularDrag = vehicleSettings.maxAngularDrag;
            }
        }
