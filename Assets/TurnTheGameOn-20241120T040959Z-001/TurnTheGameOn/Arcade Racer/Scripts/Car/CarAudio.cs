namespace TurnTheGameOn.ArcadeRacer
{
	...
	[RequireComponent(typeof (CarDriveSystem))] public class CarAudio : MonoBehaviour
	{
        private CarDriveSystem vehicleController;
        ...
        public bool setAudioLimitOnStart;
        public float engineVolumeLimitOnStart;
        public float shiftVolumeLimitOnStart;
        public float idleRevs = 0.3f;
        private float currentEngineRevs;
        ...
        void OnEnable ()
        {
            vehicleController = GetComponent<CarDriveSystem>(); // get the carcontroller ( this will not be null as we have require component)
            vehicleController.OnChangedGear += PlayShiftSound;
            if (setAudioLimitOnStart)
            {
                SetVolumeLimit(engineVolumeLimitOnStart, shiftVolumeLimitOnStart);
            }
        ...
        {
            currentEngineRevs = Mathf.Clamp(vehicleController.RevsAudio, idleRevs, 1);
            pitch = Mathf.Lerp(lowPitchMin, lowPitchMax, currentEngineRevs); // The pitch is interpolated between the min and max values, according to the car's revs.
            ...
            decFade = 1 - accFade;                
            highFade = Mathf.InverseLerp(0.2f, 0.8f, currentEngineRevs); // get the high fade value based on the cars revs
            lowFade = 1 - highFade;                
