namespace TurnTheGameOn.Timer
{
	...
	public class GameManagerEditor : Editor 
	{
		...
		public override void OnInspectorGUI()
		{
			...
			{
                EditorGUILayout.LabelField("Max Start Time: 922337193600", EditorStyles.miniLabel);
                myTarget.startTime = EditorGUILayout.DoubleField("Start Time", myTarget.startTime);
            }
			if (myTarget.timerType == Timer.TimerType.CountUp) 
			{
                EditorGUILayout.LabelField("Max Finish Time: 922337193600", EditorStyles.miniLabel);
                myTarget.finishTime = EditorGUILayout.DoubleField("Finish Time", myTarget.finishTime);
            }
			...
			if (myTarget.timerType != Timer.TimerType.CountUpInfinite)
			{
				...
				if (myTarget.loadSceneOn == Timer.LoadSceneOn.SpecificTime)
				{
					...
					myTarget.timeToLoadScene = EditorGUILayout.DoubleField ("Time to Load Scene", myTarget.timeToLoadScene);
				}
